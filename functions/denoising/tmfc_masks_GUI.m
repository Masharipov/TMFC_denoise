...
CSF_str = {'Whole-brain, GM, WM, and CSF masks are calculated based on tissue probability maps generated after segmentation. All masks are normalized to the MNI coordinate space.','',...
    ...
    '(2) The GM is obtained by applying a user-specified threshold (95% by default). This mask is used to calculate DVARS (Mascali et al., 2021).','',...
    '(3) The WM mask is obtained by applying a user-specified threshold (99% by default). Next, the WM mask is eroded several times (3 cycles by default). One erosion cycle removes one voxel from the edge of a binary mask. Finally, the WM mask is normalized to the MNI space and deprived of the brainstem (Mascali et al., 2021).','',...
    '(4) The CSF mask is obtained by applying a user-specified threshold (99% by default). To exclude voxels in close proximity to gray matter, the liberal GM mask is subtracted from the CSF mask. The liberal GM mask is obtained by applying a user-specified threshold (95% by default) and dilation (2 cycles by default). Next, the CSF mask is eroded (2 cycles by default). Finally, the CSF mask is normalized to the MNI space and deprived of any non-ventricle structure (Mascali et al., 2021).','',...
    'NOTE: BOLD signals are extracted from the intersection of the whole-brain or GM/WM/CSF masks with the implicit SPM mask created during model estimation. The implicit SPM mask includes voxels with sufficient BOLD signal intensity. By default, SPM includes voxels in the analysis if their signal is above 0.8 of the global mean signal. To create a more liberal SPM mask, the default "Masking threshold" should be reduced (e.g., to 0.4) and the models should be re-estimated prior to TMFC denoise application.'};
